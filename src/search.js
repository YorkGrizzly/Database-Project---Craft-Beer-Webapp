...
if (query.by == 'by_name') {
    request.open("GET", "http://localhost:4000/search/beer_name/" + query.name, true);
    ...
    request.open("GET", "http://localhost:4000/search/brewery_name/" + query.name, true);
    request.onload = function () {
        var result = JSON.parse(this.response);
        if (request.status >= 200 && request.status < 400) {
            if (result.data == '') {
